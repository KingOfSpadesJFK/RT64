...
namespace RT64 {
    ...
    public:
        ...
        void upscale(const UpscaleParameters &p) {
            ...
            if (p.inReactiveMask) dispatchDesc.reactive = ffxGetTextureResourceVK(&fsrContext, p.inReactiveMask->getImage(), p.inReactiveMask->getImageView(), p.inReactiveMask->getDimensions().width, p.inReactiveMask->getDimensions().height, p.inReactiveMask->getFormat(), L"inReactive");
            if (p.inLockMask) dispatchDesc.transparencyAndComposition = ffxGetTextureResourceVK(&fsrContext, p.inLockMask->getImage(), p.inLockMask->getImageView(), p.inLockMask->getDimensions().width, p.inLockMask->getDimensions().height, p.inLockMask->getFormat(), L"inTransparencyAndComposition");
    ...
    void FSR::set(QualityMode inQuality, int renderWidth, int renderHeight, int displayWidth, int displayHeight) {
        ctx->set(inQuality, renderWidth, renderHeight, displayWidth, displayHeight);
    }

    bool FSR::getQualityInformation(QualityMode quality, int displayWidth, int displayHeight, int &renderWidth, int &renderHeight) {
        return ctx->getQualityInformation(quality, displayWidth, displayHeight, renderWidth, renderHeight);
    }

    int FSR::getJitterPhaseCount(int renderWidth, int displayWidth) {
        return ctx->getJitterPhaseCount(renderWidth, displayWidth);
    }

    void FSR::upscale(const UpscaleParameters &p) {
        ctx->upscale(p);
    }

    bool FSR::isInitialized() const {
        return ctx->isInitialized();
    }

    bool FSR::requiresNonShaderResourceInputs() const {
        return false;
    }
};

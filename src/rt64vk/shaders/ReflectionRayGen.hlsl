	...
	for (uint hit = 0; hit < payload.nhits; hit++) {
		...
		if (alphaContrib >= EPSILON) {
			...
			if (usesLighting) {
				...
				resTransparent += LinearToSrgb(SrgbToLinear(hitColor.rgb) * alphaContrib * (ambientBaseColor.rgb + ambientNoGIColor.rgb + instanceMaterials[hitInstanceId].selfLight));
			}
